...
class VectorDB:
    ...
    def setup(self, collection):
        ...
        print(f"Available collections: {ls}")
        if not collection in ls:
            ...
            schema.add_field(
                field_name="key", datatype=DataType.VARCHAR, max_length=DIMENSION_TEXT
            )
            schema.add_field(
    ...
    def insert(self, key, text):
        vec = self.embed(text)
        data = {"key": key, "text": text, "embeddings": vec}
        return self.client.insert(self.collection, data)
    ...
    def vector_search(self, inp, limit=LIMIT):
        ...
        if len(cur[0]) > 0:
            for item in cur[0]:
                ...
                key = item.get("entity", {}).get("key", "")
                text = item.get("entity", {}).get("text", "")
                res.append((dist, key, text))
        return res
